...
public class ModificaCamiones extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (placa.equals("") || volumen.equals("") || peso.equals("") || estado.equals("")) {
            ...
            if (!res) {
                String message = "No se han encontrado coincidencias con el codigo o placa del camion. Intente nuevamente";
                request.setAttribute("message", message);
                request.getRequestDispatcher("ModificarCamiones.jsp").forward(request, response);
            } else {
                int resultado = cn.modificar(camion);
                if (resultado > 0) {
                    String message = "El camion ha sido modificado con exito!";
                    request.setAttribute("message", message);
                    request.getRequestDispatcher("ModificarCamiones.jsp").forward(request, response);
                } else {
